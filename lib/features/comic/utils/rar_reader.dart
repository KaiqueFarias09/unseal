...
import 'package:e_livre/features/comic/exceptions/comic_exception.dart';
...
List<RarEntry> readRarEntries(final Uint8List bytes) {
  ...
  final isRar4 = bytes[0] == 0x52 && bytes[1] == 0x61 && bytes[2] == 0x72 &&
      bytes[3] == 0x21 && bytes[4] == 0x1A && bytes[5] == 0x07 && bytes[6] == 0x00;
  final isRar5 = bytes[0] == 0x52 && bytes[1] == 0x61 && bytes[2] == 0x72 &&
...
List<RarEntry> _readRar5(final Uint8List bytes) {
  ...
  while (offset + 8 <= bytes.length) {
    ...
    if (headerType == 2) {
      ...
      final hasData =
          stored && !isDirectory && dataEnd <= bytes.length && unpackedSize == dataSize;
      entries.add(
        RarEntry(
          name: name,
          isDirectory: isDirectory,
