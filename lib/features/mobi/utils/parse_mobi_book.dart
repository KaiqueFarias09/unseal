...
import 'package:e_livre/features/mobi/entities/mobi_book.dart';
import 'package:e_livre/features/mobi/header/mobi_header.dart';
import 'package:e_livre/features/mobi/header/pdb_header.dart';
import 'package:e_livre/features/mobi/reader/mobi6_markup.dart';
import 'package:e_livre/features/mobi/reader/mobi8_reader.dart';
  ...
  if (k8i != null && k8i >= 1 && k8i - 1 < pdb.count) {
    if (_hasBoundary(pdb.record(k8i - 1))) {
      ...
      return _MobiLayout(
        ...
        [
          (
            _firstResourceIndex(header.firstImageIndex, header.textRecordCount, 1),
            k8i - 1,
          ),
          (
            _firstResourceIndex(kf8FirstImage, kf8Header.textRecordCount, textOffset),
            pdb.count,
          ),
        ],
...
MobiBook _parseMobi6(final PdbHeader pdb, final MobiHeader header) {
  ...
  for (var i = 1;
      i <= header.textRecordCount && i < pdb.count;
      i++) {
    processed.add(i);
  }
...
BinaryFile? _readCoverRecord(
  ...
  final candidates = <int>[
    if (coverOffset != null) base + coverOffset,
    base,
  ];
  for (final index in candidates) {
    if (index < 0 || index >= pdb.count) {
      continue;
    }
