...
class ExthHeader {
  ...
  factory ExthHeader.parse(final Uint8List raw, final String codec, final String headerTitle) {
    if (raw.length < 12 ||
        raw[0] != 0x45 || raw[1] != 0x58 || raw[2] != 0x54 || raw[3] != 0x48) {
  ...
  int? int32(final int id) {
    final values = _records[id];
    if (values == null || values.isEmpty || values.first.length < 4) {
      return null;
    }
  ...
  int? get coverOffset {
    ...
    if (offset == null || offset == 0xFFFFFFFF) {
      return null;
    }
    return offset;
  }
  ...
  int? get kf8HeaderIndex {
    ...
    if (index == null || index == 0xFFFFFFFF) {
      return null;
    }
    return index;
  }
