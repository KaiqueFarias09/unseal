...
BookMetadata epubBookMetadata(
  ...
  return BookMetadata(
    ...
    authors: [
      if (metadata.creator?.isNotEmpty == true) metadata.creator!,
    ],
    languages: [
    ...
    subjects: [
      if (metadata.subject?.isNotEmpty == true) metadata.subject!,
    ],
...
String? _findIsbn(final List<String> identifiers) {
  for (final identifier in identifiers) {
    ...
    final isIsbn10 = compact.length == 10 &&
        RegExp(r'^\d{9}[\dX]$').hasMatch(compact);
    final isIsbn13 = compact.length == 13 &&
        RegExp(r'^97[89]\d{10}$').hasMatch(compact);
