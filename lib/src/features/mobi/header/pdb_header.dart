...
class PdbHeader implements PdbRecordAccess {
  ...
  PdbHeader.parse(final Uint8List bytes) {
    if (bytes.length < 78) throw const InvalidBookException('File is too small to be a MOBI book.');
  ...
  int recordLength(final int index) {
    if (index == recordCount - 1) return _bytes.length - offsets[index];

    final next = offsets[index + 1];
    if (next > offsets[index]) return next - offsets[index];
    // Records stored out of order: use the nearest greater offset.
    var end = _bytes.length;
    for (final offset in offsets) {
      if (offset > offsets[index] && offset < end) end = offset;
    }
