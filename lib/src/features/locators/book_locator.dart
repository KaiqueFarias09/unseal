...
///
/// A locator addresses one point or range of a book in exactly one of
/// three coordinate systems:
///
/// * [TextLocator] — character offsets in the canonical
///   `documentText` space of one reading-order section, the same
///   space search hits and CFI character offsets address;
/// * [CfiLocator] — an opaque EPUB CFI string;
/// * [PageLocator] — a flat page index for page-based formats such
///   as comics (and, later, PDF).
///
/// The union is sealed, so consumers can switch over all kinds
/// exhaustively. `locator_codec.dart` versioned-encodes every kind
/// to JSON and back, and `fuzzy_relocation.dart` re-anchors a
/// [TextLocator] whose offsets drifted.
library;

/// Base of the locator union: [TextLocator], [CfiLocator] or
/// [PageLocator].
sealed class BookLocator {
  const BookLocator();
}

/// A position or range in the document-text space of one
...
final class TextLocator extends BookLocator {
  ...
  /// One past the last character offset of the located range
  /// (exclusive); equals [start] for a point.
  final int end;

  /// The quoted text around the range, when known — the payload
  /// `fuzzy_relocation.dart` needs to re-anchor these offsets after
  /// the section text changed.
...
final class CfiLocator extends BookLocator {
  ...
  /// The CFI string — e.g. `epubcfi(/6/4!/4/2:10)` — addressing a
  /// point or a comma-separated range; parsed and resolved by the
  /// `cfi` feature.
  final String cfi;
...
/// A position addressed by flat page index, for page-based formats
/// such as comics (and, in the future, PDF).
final class PageLocator extends BookLocator {
  /// Creates a page locator; [total] carries the page count when the
  /// format exposes one.
  ...
  @override
  bool operator ==(final Object other) =>
      other is PageLocator && other.pageIndex == pageIndex && other.total == total;

  @override
  String toString() => total == null ? 'PageLocator($pageIndex)' : 'PageLocator($pageIndex/$total)';
}

/// The exact text of a located range together with its immediate
/// surrounding context — the W3C TextQuoteSelector analogue that lets
/// `fuzzy_relocation.dart` re-anchor a [TextLocator] when the section
/// text changed under its offsets.
final class TextQuote {
  ...
  @override
  bool operator ==(final Object other) =>
      other is TextQuote && other.before == before && other.text == text && other.after == after;
