/// Simple-font encoding tables: byte code to Unicode.
///
/// The three Adobe encodings a PDF simple font can name (PDF
/// 32000-1:2008 §D), derived from the standard glyph-name tables
/// (as published by pdf.js, Apache-2.0; data only).
library;

/// The `/StandardEncoding` table (256 entries; null = no glyph).
const List<String?> pdfStandardEncoding = <String?>[
  null,
  null,
  null,
  null,
...
/// The `/WinAnsiEncoding` table (256 entries; null = no glyph).
const List<String?> pdfWinAnsiEncoding = <String?>[
  null,
  null,
  null,
  null,
...
/// The `/MacRomanEncoding` table (256 entries; null = no glyph).
const List<String?> pdfMacRomanEncoding = <String?>[
  null,
  null,
  null,
  null,
...
/// A glyph-name to Unicode resolver for `/Encoding /Differences`.
///
/// Covers the Adobe Glyph List (data as published by pdf.js,
/// Apache-2.0) plus the `uniXXXX`/`uXXXX[XX]` escape forms of the
/// PDF spec; unknown names resolve to null.
String? pdfGlyphToUnicode(final String name) => _glyphs[name] ?? _uniEscape(name);

String? _uniEscape(final String name) {
  if (name.startsWith('uni') && name.length >= 7) {
    final parts = <int>[];
    for (var i = 3; i + 4 <= name.length; i += 4) {
      final value = int.tryParse(name.substring(i, i + 4), radix: 16);
  ...
  if (name.startsWith('u') && name.length >= 5) {
    final digits = name.substring(1).split('_').first;
    final value = int.tryParse(digits, radix: 16);

    return value == null || value > 0x10FFFF ? null : String.fromCharCode(value);
  }

  return null;
}

final Map<String, String> _glyphs = <String, String>{
  'A': '\u0041',
  'AE': '\u00C6',
  'AEacute': '\u01FC',
  'AEmacron': '\u01E2',
...
/// Builds a 256-entry lookup table over [base] applying
/// [differences] entries (code -> glyph name); unmapped codes
/// resolve to the replacement character so callers never see null.
List<String> applyEncodingDifferences(
  final List<String?> base,
  final Map<int, String> differences,
) {
  final table = List<String>.filled(256, '\uFFFD');
  for (var code = 0; code < 256; code++) {
    final mapped = base[code];
    if (mapped != null && mapped.isNotEmpty) table[code] = mapped;
  }
  differences.forEach((final code, final name) {
    if (code < 0 || code > 255) return;
    final unicode = pdfGlyphToUnicode(name);
    table[code] = unicode == null || unicode.isEmpty ? '\uFFFD' : unicode;
  });
