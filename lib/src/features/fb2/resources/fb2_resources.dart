...
final class Fb2Resources {
  const Fb2Resources._(this.binaries, this.coverId, this.cover);

  /// Decodes all valid `<binary>` elements under [root].
  factory Fb2Resources.fromRoot(final XmlElement root) {
    final binaries = <String, BinaryFile>{};
    for (final element in root.findElements('binary')) {
      final id = element.getAttribute('id');
      if (id == null || id.isEmpty) continue;

      final binary = decodeFb2Binary(
        id,
    ...
    return Fb2Resources.fromBinaries(root, binaries);
  }

  /// Resolves resource semantics for an already decoded binary subset.
  factory Fb2Resources.fromBinaries(final XmlElement root, final Map<String, BinaryFile> binaries) {
    final coverId = fb2CoverId(root);

    return Fb2Resources._(
      binaries,
      coverId,
  ...
  Map<String, String> get extensionMap => <String, String>{
...
BinaryFile? decodeFb2Binary(final String id, final String contentType, final String base64Text) {
...
String? fb2CoverId(final XmlElement root) {
...
String? _imageHref(final XmlElement image) =>
    image.getAttribute('href', namespace: _xlinkNamespace) ??
