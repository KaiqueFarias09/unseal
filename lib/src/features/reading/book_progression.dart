...
///
/// The canonical text space of a section is the `documentText` of its HTML file — the same space
/// search, CFI and navigation anchors address. Character lengths are computed once at construction
/// (`documentTextOf` is memoized per file, so the pass is cheap and shared with other consumers).
/// Non-HTML sections (comic pages) count as length 0 but keep their index, so section indexes stay
/// aligned with `Book.readingOrder`.
final class BookProgression {
  /// Builds the progression of [book], measuring every HTML reading order section once.
  factory BookProgression.of(final Book book) {
    ...
    for (final section in sections) {
      starts.add(total);
      final file = section.isHtml ? filesByPath[section.name] : null;
      final length = file == null ? 0 : documentTextOf(file).length;
      lengths.add(length);
      total += length;
    }
  ...
  /// Total characters of canonical text across all sections. Fully non-text books (comics) measure
  /// 0.
  int get totalCharacters => _totalCharacters;

  /// Progress across the whole book at [charOffset] inside [sectionIndex]: the characters before
  /// the section plus the in-section offset, over [totalCharacters].
  ///
  /// Negative positions measure 0 and positions past the last section measure 1; inside the reading
  /// order, [charOffset] clamps to the section's length. A fully non-text book (comic) falls back
  /// to [sectionIndex] over [sectionCount].
  ...
  /// Progress within [sectionIndex] at [charOffset]: 0 at the section's first character, 1 at its
  /// end (or past it).
  ///
  /// [charOffset] clamps to the section's length; empty sections (non-HTML entries) and queries
  /// outside the reading order measure
  /// 0.
