...
final class SyntheticBook extends Book {
  ...
  @override
  List<ReadingOrderItem> get readingOrder => _order.isEmpty
      ? <ReadingOrderItem>[
...
/// named by index.
Files htmlFiles(final List<String> contents) => Files(
  images: const <BinaryFile>[],
  css: const <TextFile>[],
  html: <TextFile>[
    for (var i = 0; i < contents.length; i++)
      TextFile(name: 'section$i.html', type: 'html', path: 'section$i.html', content: contents[i]),
  ],
...
Navigation navigationOf(final List<NavPoint> points) =>
    Navigation(title: 'test', navPoints: points);
