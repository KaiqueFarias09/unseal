...
void main() {
  group('BookReader dispatcher', () {
    ...
    test('opens a MOBI 6 book into a MobiBook', () async {
      final book = await BookReader.openFromPath('test/resources/books/mobi/alice-old-pg.mobi');
      expect(book, isA<MobiBook>());
      expect(book.format, BookFormat.mobi);
    });

    test('opens an AZW3 book into a MobiBook', () async {
      final book = await BookReader.openFromPath('test/resources/books/mobi/alice-kf8-pg.azw3');
      expect(book, isA<MobiBook>());
    ...
    test('opens a CBZ comic into a ComicBook', () async {
      final book = await BookReader.openFromPath('test/resources/books/comic/synthetic-pages.cbz');
    ...
    test('opens a PDF into a reflowable PdfBook', () async {
      final book = await BookReader.openFromPath('test/resources/books/pdf/dickens-sample.pdf');
